...
// Updates the authentication link based on the current user status
function updateAuthLink() {
    const user = getCurrentUser();
    const authLink = document.getElementById('login-logout-link');
    const navbar = document.getElementById('navbar');
    
    const existingWelcome = document.getElementById('welcome-msg');
    if (existingWelcome) existingWelcome.remove();
    
    if (authLink) {
        authLink.textContent = user ? 'Logout' : 'Login';
        authLink.href = user ? 'javascript:logout();' : 'home.html';
    }
    
    if (user && navbar && !document.getElementById('welcome-msg')) {
        const welcomeSpan = document.createElement('span');
        welcomeSpan.id = 'welcome-msg';
        welcomeSpan.textContent = `Welcome, ${user.username}`;
        welcomeSpan.style.marginLeft = '20px';
